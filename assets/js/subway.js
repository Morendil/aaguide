...
//        if ((canvas[0].getContext) || (typeof(G_vmlCanvasManager) != 'undefined'))
    ...
    _getCanvasLayer: function (el, overlay) {
        this.layer++;
        var canvas = $(document.createElement("canvas")).attr("style","position:absolute;z-Index:" + ((overlay ? 2000 : 1000) + this.layer)).attr("width",this.options.pixelWidth).attr("height",this.options.pixelHeight);

        el.append(canvas);

        if (typeof(G_vmlCanvasManager) != 'undefined')            
	{
	   ieCanvas = G_vmlCanvasManager.initElement(canvas[0]);
	   return (ieCanvas.getContext("2d"));
	}
	else
        ...
        if (this._supportsCanvas()) {
            ...
            $(el).children("ul").each(function (index) {
                ...
                $(ul).children("li").each(function () {
                    ...
                    var y = "";
                    if (coords.indexOf(",") > -1) {
                        x = Number(coords.split(",")[0]) + shiftX;
                        y = Number(coords.split(",")[1]) + shiftY;
                    }
	...
	{
	     if (this.options.altImage)
	     {
		$(el).html("<div>(Your browser does not support the interactive version of the visualization, so a static image is displayed.)</div><img src='" + this.options.altImage + "' />");
	     }
	}
    },
    ...
    _drawMarker: function (el, ctx, scale, color, textClass, width, data, reverseMarkers) {
        ...
        var style = (textClass != "" ? "class='" + textClass + "' " : "") + "style='" + (textClass == "" ? "font-size:8pt;font-family:Verdana,Arial,Helvetica,Sans Serif;text-decoration:none;" : "") + 
			"width:100px;" + (pos != "" ? pos : "") + 
			";position:absolute;top:" + (y + 0 - (topOffset > 0 ? topOffset : 0)) + "px;left:" + (x + 0) + "px;z-index:3000;'";
        if (data.link != "")
